			...
			if(xhr.readyState == 4){
				if(xhr.status == 200){
					if(xhr.responseText == 1){
						...
						document.getElementById("span1").innerHTML = "用户名被占用了";
					}else if(xhr.responseText == 2){
						document.getElementById("span1").innerHTML = "用户名可以使用";
					}else{
						document.getElementById("span1").innerHTML = "抱歉！系统出现异常！";
					}
				}
			}
		}
		// 3.打开连接
		xhr.open("get","/user/check/"+username+"/1?time="+new Date().getTime(),true);
		// 4.发送
		xhr.send(null);
	}
		...
		xhr.onreadystatechange = function(){
			if(xhr.readyState == 4){
				if(xhr.status == 200){
					if(xhr.responseText == 1){
						//document.getElementById("span1").innerHTML = xhr.responseText;
						document.getElementById("span2").innerHTML = "手机号已经被注册";
					}else if(xhr.responseText == 2){
						document.getElementById("span2").innerHTML = "手机号可以使用";
					}else{
						document.getElementById("span2").innerHTML = "抱歉！系统出现异常！";
					}
				}
			}
		}
		// 3.打开连接
		xhr.open("GET","/user/check/"+phone+"/2?time="+new Date().getTime(),true);
		// 4.发送
		xhr.send(null);
	}
		...
		xhr.onreadystatechange = function(){
			if(xhr.readyState == 4){
				if(xhr.status == 200){
					if(xhr.responseText == 1){
						//document.getElementById("span1").innerHTML = xhr.responseText;
						document.getElementById("span3").innerHTML = "邮箱已经被注册";
					}else if(xhr.responseText == 2){
						document.getElementById("span3").innerHTML = "邮箱号可以使用";
					}else{
						document.getElementById("span3").innerHTML = "抱歉！系统出现异常！";
					}
				}
			}
		}
		// 3.打开连接
		xhr.open("GET","/user/check/"+email+"/3?time="+new Date().getTime(),true);
		// 4.发送
		xhr.send(null);
	}
	...
	<div class="span5">
		<div class="logo">
			<a href="http://localhost:8082/">
				<img src="/images/taotao-logo.gif" alt="传智播客" style="width: 218px;"/>
			</a>
		</div>
	</div>
	<div class="span9">
<div class="headerAd">
					<img src="/images/header.jpg" width="320" height="55" alt="正品保障" title="正品保障"/>
</div>	</div>
		...
		<div class="span24">
			<div class="wrap">
				<div class="main clearfix">
					...
					<form id="registerForm" action="/user/register"  method="post" novalidate="novalidate" onsubmit="return checkForm();">
						<table>
							<tbody><tr>
								<th>
							...
							<tr>
								...
								<tr>
									...
									<td>
										<input type="text" id="email" name="email" class="text" onblur="checkEmail()"/>
										<span id="span3"></span>
									</td>
								</tr>
							<tr>
